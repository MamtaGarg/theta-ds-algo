...
Return_ the number of the room that held the most meetings. If there are multiple rooms, return the room with the lowest number._
...
public class MeetingRoomsIII {
    ...
    public static int mostBooked(int n, int[][] meetings) {
        ...
        // Step 2: Available rooms (min heap by room number)
        PriorityQueue<Integer> availableRooms = new PriorityQueue<>();
        for (int i = 0; i < n; i++) {
            availableRooms.offer(i);
        }

        // Step 3: Occupied rooms (min heap by end time)
        PriorityQueue<long[]> occupiedRooms = new PriorityQueue<>(
                (a, b) -> Long.compare(a[0], b[0]) // sort by endTime
        );
        ...
        for (int[] meeting : meetings) {
            ...
            // Step 4: Free rooms that are done before current start
        ...
        // Step 5: Find room with max meetings
        int maxRoom = 0;
        for (int i = 1; i < n; i++) {
